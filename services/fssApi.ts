...
export class FinancialServicesCommissionAPI {
    ...
    private readonly BASE_URL = 'https://apis.data.go.kr/1160100/service/GetStockSecuritiesInfoService';
     ...
     * API URL 생성
     */
    private buildApiUrl(stockCode: string, startDate: string, endDate: string): string {
    ...
    private parseXmlError(xmlText: string): string {
        try {
            ...
            const returnAuthMsgMatch = xmlText.match(/<returnAuthMsg>(.*?)<\/returnAuthMsg>/);
            const returnReasonCodeMatch = xmlText.match(/<returnReasonCode>(.*?)<\/returnReasonCode>/);
        ...
        try {
            ...
            if (response.status === 500) {
                ...
                if (responseText.includes('SERVICE_KEY_IS_NOT_REGISTERED_ERROR')) {
                    throw new Error('API 키가 등록되지 않았거나 서비스 신청이 승인되지 않았습니다. 공공데이터포털에서 "금융위원회_주식시세정보" 서비스를 신청하고 승인을 받아주세요.');
                }
                
                if (responseText.includes('INVALID_REQUEST_PARAMETER_ERROR')) {
                    throw new Error('잘못된 요청 파라미터입니다. 날짜 형식이나 종목코드를 확인해주세요.');
                }
                ...
                throw new Error(`서버 내부 오류 (HTTP 500): 공공데이터포털 API 서버에 문제가 있습니다. 잠시 후 다시 시도해주세요.`);
            }
            ...
            try {
                ...
                console.error('JSON 파싱 실패. 응답 내용:', responseText.substring(0, 500));
                throw new Error('예상치 못한 응답 형식입니다. API 서버에서 JSON이 아닌 응답을 반환했습니다.');
            }
            
            console.log('API 응답 상태:', data.response?.header?.resultCode, data.response?.header?.resultMsg);
            ...
            if (data.response?.header?.resultCode !== '00') {
                ...
                // 일반적인 오류 코드들 처리
                if (resultMsg.includes('SERVICE_KEY_IS_NOT_REGISTERED_ERROR') || resultMsg.includes('30')) {
                    throw new Error('API 키가 등록되지 않았거나 서비스 신청이 승인되지 않았습니다.');
                }
                
                if (resultMsg.includes('NO_DATA')) {
                    throw new Error('해당 기간에 주가 데이터가 없습니다. 다른 날짜 범위를 시도해보세요.');
                }
    ...
    async testApiKey(): Promise<boolean> {
        try {
            // 테스트용 URL 생성
            const testUrl = `${this.BASE_URL}/getStockPriceInfo?serviceKey=${encodeURIComponent(this.API_KEY)}&numOfRows=1&pageNo=1&resultType=json&likeSrtnCd=005930&basDt=20240101`;
    ...
    getSupportedStocks(): string[] {
        return [
            ...
            'LG에너지솔루션', 'POSCO', 'LG화학', '카카오', '셀트리온',
            'KB금융', '신한지주', '한국전력', 'LG디스플레이', '아모레퍼시픽',
            '삼성바이오로직스', '카카오뱅크', '크래프톤', '하이브', 'HMM', '한화솔루션'
        ];
    }
}
