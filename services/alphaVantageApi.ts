  ...
  private parseApiResponse(
    ...
    if (!data["Time Series (Daily)"]) {
      ...
      if ((data as any)["Error Message"]) {
        throw new Error(
          `Alpha Vantage 오류: ${(data as any)["Error Message"]}`
        );
      }
      throw new Error(
        "주가 데이터를 찾을 수 없습니다. 종목 심볼을 확인해주세요."
      );
    }
    ...
    if (stockData.length === 0) {
      throw new Error(
        "지정된 날짜 범위에 주가 데이터가 없습니다. 주말이나 공휴일이 아닌 거래일을 선택해주세요."
      );
    }
  ...
  async fetchStockData(
    ...
    try {
      ...
      console.log(
        ...
        apiUrl.replace(this.API_KEY, "[API_KEY]")
      );
