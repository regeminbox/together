  ...
  async fetchStockData(
    ...
    try {
      ...
      if (isKorean) {
        ...
        try {
          const data = await this.fssApi.fetchStockData(
            symbol,
            startDate,
            endDate
          );
          ...
          if (
            errorMessage.includes("API 키가 등록되지 않았습니다") ||
            errorMessage.includes("SERVICE_KEY_IS_NOT_REGISTERED_ERROR")
          ) {
            return {
              success: false,
...
• 최근 1-2주 기간으로 설정해보세요"`,
        ...
        try {
          const data = await this.alphaVantageApi.fetchStockData(
            symbol,
            startDate,
            endDate
          );
  ...
  async testApis(): Promise<{
    ...
    return {
      ...
      alphaVantage:
        alphaVantageResult.status === "fulfilled"
          ? alphaVantageResult.value
          : false,
    };
  }
    ...
    if (isKorean) {
      ...
      return {
        ...
        suggestion: isSupported
          ? undefined
          : "지원하는 한국 주식: " +
            supportedStocks.korean.slice(0, 5).join(", ") +
            " 등",
      };
    } else {
      ...
      return {
        ...
        suggestion: isSupported
          ? undefined
          : "지원하는 미국 주식: " +
            supportedStocks.us.slice(0, 5).join(", ") +
            " 등",
      };
    }
  }
