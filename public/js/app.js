...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("start-date").value = threeDaysAgo
    .toISOString()
    .split("T")[0];
...
function displayStockAnalysis(data) {
  ...
  const changeClass =
    direction === "up" ? "positive" : direction === "down" ? "negative" : "";
  ...
  let formattedPrice;
  if (isKorean) {
    formattedPrice = `${changeSymbol}${currency}${Math.abs(
      data.priceChange.absolute
...
// ===== 기업분석보고서 ========
async function performCompanyAnalysis() {
  const symbol = document
    .querySelector("#analysis-tab #stock-symbol")
    .value.trim();
  const resultContainer = document.getElementById("analysis-result-container");
  const resultDiv = document.getElementById("analysis-result");
  const loadingDiv = document.getElementById("stock-loading"); // 재사용
